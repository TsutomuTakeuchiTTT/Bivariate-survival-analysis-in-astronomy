#!/usr/bin/env python3
"""
Supplementary material: reference implementation of Algorithm 3
for the paper
...
Purpose
-------
This script is the reference implementation of Algorithm 3 in the paper.  It
contains the same estimator as ``bivariate_survival_data_io.py``, together with
mock-data generation and internal validation routines.  It is intended for
methodological verification, reproducibility checks, and comparison with the
mathematical definitions in Sections 3.6--3.8 of the manuscript.

Design principles
-----------------
1. The manuscript is treated as the formal specification.
2. The Dabrowska jump set is J_n = U_n x V_n, where U_n and V_n are the
   marginal event-time sets. It is not replaced by the set of observed paired
   rows.
3. Weighted risk sets and weighted counting processes are computed by
   histogramming and cumulative sums, then verified against direct brute-force
   definitions in the self-test.
4. Product-integral evaluation is performed by prefix sums of log(1 + Gamma),
   avoiding query-by-jump matrices.
5. The gridded CDF written for diagnostics can be repaired through
   nonnegative cell masses so that rectangle positivity holds on the grid.
   This repair is only for gridded diagnostic output; it is not part of the
   jump-set estimator definition.
6. Diagnostic figures are generated from saved numerical outputs and are not
   part of the estimator definition.
7. The code favors readability, auditability, and paper-code correspondence
   over micro-optimization.

Input columns for a user-supplied CSV
-------------------------------------
The same input format as ``bivariate_survival_data_io.py`` is accepted:

    x1_obs, x2_obs, y1, y2, delta1, delta2, region

Optional simulation-truth columns are
...
Run built-in validation tests:
    ...
    python bivariate_survival_reference.py --self-test --no-plots

Run the built-in mock-data demonstration:

    python bivariate_survival_reference.py --outdir reference_outputs

Run Algorithm 3 on a user CSV using the reference implementation:

    python bivariate_survival_reference.py \
        --input example/example_input.csv \
        --outdir reference_outputs

Jupyter notebook use
--------------------
The command-line parser uses ``parse_known_args`` so that Jupyter/IPython
kernel arguments are ignored.  To pass arguments manually in a notebook, call
``main([...])`` explicitly after loading the file.
"""
...
def brute_force_weighted_dabrowska_jump_table(
    df_abc: pd.DataFrame,
    weights: Optional[Array] = None,
    eps: float = 1e-12,
) -> pd.DataFrame:
    """Direct O(n |U| |V|) implementation used only for small tests."""
    _check_required_columns(df_abc)
    dat = df_abc[df_abc["region"].isin(["A", "B", "C"])].copy().reset_index(drop=True)
    z1 = -dat["x1_obs"].to_numpy(float)
    z2 = -dat["x2_obs"].to_numpy(float)
    ...
    for u in U:
        for v in V:
            risk = float(np.sum(w[(z1 >= u) & (z2 >= v)]))
            num11 = float(np.sum(w[(z1 == u) & (z2 == v) & d1 & d2]))
            num10 = float(np.sum(w[(z1 == u) & (z2 >= v) & d1]))
            num01 = float(np.sum(w[(z1 >= u) & (z2 == v) & d2]))

            if risk <= eps:
                dL11 = dL10 = dL01 = gamma = 0.0
            else:
                dL11 = float(np.clip(num11 / risk, 0.0, 1.0 - eps))
                dL10 = float(np.clip(num10 / risk, 0.0, 1.0 - eps))
                dL01 = float(np.clip(num01 / risk, 0.0, 1.0 - eps))
                denom = max((1.0 - dL10) * (1.0 - dL01), eps)
                gamma = float(max((dL11 - dL10 * dL01) / denom, -1.0 + eps))

            rows.append({
                "z1": float(u), "z2": float(v), "risk_w": risk,
                "num10_w": num10, "num01_w": num01, "num11_w": num11,
                "DeltaLambda10_w": dL10, "DeltaLambda01_w": dL01,
                "DeltaLambda11_w": dL11, "Gamma_w": gamma, "Q_w": gamma,
                "log1p_Q_w": float(_safe_log1p(gamma, floor=eps)),
            })
    return pd.DataFrame(rows)


def brute_force_survival_z(est: WeightedDabrowskaJumpEstimator, z1q, z2q) -> Array:
    """Direct product-integral evaluation used only for small tests."""
    z1q = np.asarray(z1q, dtype=float)
    z2q = np.asarray(z2q, dtype=float)
    bshape = np.broadcast_shapes(z1q.shape, z2q.shape)
    q1 = np.broadcast_to(z1q, bshape).ravel()
    ...
    jt = est.jump_table
    j1 = jt["z1"].to_numpy(float)
    j2 = jt["z2"].to_numpy(float)
    log1p_q = jt["log1p_Q_w"].to_numpy(float)

    for k in range(len(q1)):
        log_s = 0.0
        log_s += est._marginal_log_factor(est.margin1_table, np.array([q1[k]]))[0]
        log_s += est._marginal_log_factor(est.margin2_table, np.array([q2[k]]))[0]
        mask = (j1 <= q1[k]) & (j2 <= q2[k])
        log_s += float(np.sum(log1p_q[mask]))
        out[k] = np.exp(log_s)

    return np.clip(out.reshape(bshape), 0.0, 1.0)


def run_self_tests(seed: int = 123) -> dict:
    """Run small deterministic validation tests."""
    df = make_mock_abc(n=80, seed=seed, rho=0.55)
    est = weighted_dabrowska_jump_estimator(df)
    fast = est.jump_table.reset_index(drop=True)
    brute = brute_force_weighted_dabrowska_jump_table(df).reset_index(drop=True)

    cols = [
        "risk_w", "num10_w", "num01_w", "num11_w",
        "DeltaLambda10_w", "DeltaLambda01_w", "DeltaLambda11_w",
        "Gamma_w", "log1p_Q_w",
    ]
    max_abs = {c: float(np.max(np.abs(fast[c].to_numpy(float) - brute[c].to_numpy(float)))) for c in cols}
    jump_match = all(v < 1e-10 for v in max_abs.values())

    z1_grid = np.linspace(-df["x1_obs"].quantile(0.95), -df["x1_obs"].quantile(0.05), 10)
    z2_grid = np.linspace(-df["x2_obs"].quantile(0.95), -df["x2_obs"].quantile(0.05), 10)
    Z1, Z2 = np.meshgrid(z1_grid, z2_grid, indexing="ij")
    S_fast = est.survival_z(Z1, Z2)
    S_brute = brute_force_survival_z(est, Z1, Z2)
    prod_diff = float(np.max(np.abs(S_fast - S_brute)))

    raw_grid = np.array([[0.2, 0.4], [0.5, 0.45]])
    repaired_grid = _cdf_grid_repair(raw_grid, enforce_rectangle=True)
    rectangle_diag = _grid_diagnostics(repaired_grid)

    return {
        "n_ABC": int(len(df)),
        "n_jump_rows": int(len(est.jump_table)),
        "jump_table_bruteforce_match": bool(jump_match),
        "product_integral_bruteforce_match": bool(prod_diff < 1e-10),
        "max_abs_jump_table_differences": max_abs,
        "max_abs_product_integral_difference": prod_diff,
        "cdf_repair_rectangle_positive": bool(rectangle_diag["rectangle_positive"]),
        "cdf_repair_negative_cells": int(rectangle_diag["rectangle_negative_cells"]),
    }


# =============================================================================
# Mock data and output helpers
# =============================================================================

def make_mock_abc(n: int = 500, seed: int = 123, rho: float = 0.55) -> pd.DataFrame:
    """Generate a mock A/B/C/D catalogue and return the observed A+B+C subset."""
    rng = np.random.default_rng(seed)
    mean = np.array([10.5, 10.2])
    sd1, sd2 = 0.55, 0.65
    rho = float(np.clip(rho, -0.98, 0.98))
    cov = np.array([[sd1**2, rho * sd1 * sd2], [rho * sd1 * sd2, sd2**2]])
    x = rng.multivariate_normal(mean, cov, size=n)

    r = rng.uniform(0.0, 1.0, size=n)
    y1 = 9.25 + 1.35 * r + rng.normal(0, 0.10, n)
    y2 = 9.10 + 1.55 * r + rng.normal(0, 0.10, n)

    d1 = x[:, 0] >= y1
    d2 = x[:, 1] >= y2

    region = np.full(n, "D", dtype=object)
    region[d1 & d2] = "A"
    region[d1 & ~d2] = "B"
    region[~d1 & d2] = "C"

    x1_obs = np.where(d1, x[:, 0], y1)
    x2_obs = np.where(d2, x[:, 1], y2)
    ...
    return df[df["region"].isin(["A", "B", "C"])].reset_index(drop=True)
...
def main(argv: Optional[list[str]] = None) -> None:
    parser = argparse.ArgumentParser(description="Algorithm 3 reference implementation")
    parser.add_argument("--input", type=str, default=None, help="Optional input CSV")
    parser.add_argument("--outdir", type=str, default="bivariate_survival_outputs", help="Output directory")
    parser.add_argument("--n-mock", type=int, default=500, help="Mock parent sample size")
    parser.add_argument("--seed", type=int, default=2026, help="Random seed for mock data/self-tests")
    parser.add_argument("--mock-rho", type=float, default=0.55, help="Correlation coefficient for built-in mock data")
    ...
    parser.add_argument("--self-test", action="store_true", help="Run built-in validation tests first")
    ...
    if args.self_test:
        validation = run_self_tests(seed=args.seed)
        print(json.dumps(validation, indent=2))
        if (
            not validation["jump_table_bruteforce_match"]
            or not validation["product_integral_bruteforce_match"]
            or not validation["cdf_repair_rectangle_positive"]
        ):
            raise RuntimeError("Self-test failed.")

    if args.input is None:
        df = make_mock_abc(n=args.n_mock, seed=args.seed, rho=args.mock_rho)
    else:
        df = pd.read_csv(args.input)
        _check_required_columns(df)
        df = df[df["region"].isin(["A", "B", "C"])].reset_index(drop=True)

    x1_grid = np.linspace(df["x1_obs"].quantile(0.05), df["x1_obs"].quantile(0.95), args.grid)
    x2_grid = np.linspace(df["x2_obs"].quantile(0.05), df["x2_obs"].quantile(0.95), args.grid)
